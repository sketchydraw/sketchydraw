...
+ (NSData *)dataWithBase32String:(NSString *)encoded
{
	...
	size_t charDigitsLen = sizeof(charDigits);
...
{
	NSBundle *bundle = [NSBundle bundleForClass:[ASWebImageRep class]]; // [self class]では駄目。[self class]はNSConreteData //
	NSString *tableName = @"ASWebViewError";
	NSString *errorDomain = @"CocoaDevUserAdditionsDomain";
	if ([self length] == 0) return self;
	...
	if (inflateInit2(&strm, (15+32)) != Z_OK) {
		if ((NSError **)NULL != outError) {
			NSInteger code = 3;
			NSDictionary *errorUserInfo = [NSDictionary dictionaryWithObjectsAndKeys:NSLocalizedStringFromTableInBundle(@"gzipInflate could not initialize.",tableName,bundle,@""),NSLocalizedDescriptionKey, NSLocalizedStringFromTableInBundle(@"gzipInflate could not initialize.",tableName,bundle,@""), NSLocalizedFailureReasonErrorKey,NSLocalizedStringFromTableInBundle(@"Make sure that gzip format.",tableName,bundle,@""),NSLocalizedRecoverySuggestionErrorKey, nil];
			*outError = [NSError errorWithDomain:errorDomain code:code userInfo:errorUserInfo];
		}
		return nil;		
	}
	while (NO == done)
	...
	if (inflateEnd (&strm) != Z_OK) {
		if ((NSError **)NULL != outError) {
			NSInteger code = 2;
			NSDictionary *errorUserInfo = [NSDictionary dictionaryWithObjectsAndKeys:NSLocalizedStringFromTableInBundle(@"gzipInflate could not end.",tableName,bundle,@""),NSLocalizedDescriptionKey, NSLocalizedStringFromTableInBundle(@"gzipInflate could not end.",tableName,bundle,@""), NSLocalizedFailureReasonErrorKey,NSLocalizedStringFromTableInBundle(@"Make sure that gzip format.",tableName,bundle,@""),NSLocalizedRecoverySuggestionErrorKey, nil];
			*outError = [NSError errorWithDomain:errorDomain code:code userInfo:errorUserInfo];
		}
		return nil;
	}
	...
	if (YES == done)
	{
		...
		if ((NSError **)NULL != outError) {
			NSInteger code = 1;
			NSDictionary *errorUserInfo = [NSDictionary dictionaryWithObjectsAndKeys:NSLocalizedStringFromTableInBundle(@"gzipInflate could not complete.",tableName,bundle,@""),NSLocalizedDescriptionKey,NSLocalizedStringFromTableInBundle(@"gzipInflate could not complete.",tableName,bundle,@""), NSLocalizedFailureReasonErrorKey,NSLocalizedStringFromTableInBundle(@"Make sure that gzip format.",tableName,bundle,@""),NSLocalizedRecoverySuggestionErrorKey, nil];
			*outError = [NSError errorWithDomain:errorDomain code:code userInfo:errorUserInfo];
		}
		return nil;
	}
...
// --------------------------------CRC32-------------------------------
static const NSUInteger crc32table[] =
{
...
// MD5_Init(),SHA1_Init()など10.7でdeprecated。 //
// Hash function, by DamienBob

/* #define HEComputeDigest(method)			method##_CTX ctx; \
	unsigned char digest[method##_DIGEST_LENGTH]; \
	method##_Init(&ctx); \
	method##_Update(&ctx, [self bytes], [self length]); \
	method##_Final(digest, &ctx);

#define HEComputeDigestNSData(method)	HEComputeDigest(method) \
	return [NSData dataWithBytes:digest length:method##_DIGEST_LENGTH];

#define HEComputeDigestNSString(method)	static char __HEHexDigits[] = "0123456789abcdef"; \
